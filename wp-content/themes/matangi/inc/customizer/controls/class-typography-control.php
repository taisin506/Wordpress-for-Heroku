...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Matangi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Matangi_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'matangi-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'matangi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), MATANGI_VERSION, true );
			wp_enqueue_style( 'matangi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), MATANGI_VERSION );

			wp_enqueue_script( 'matangi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'matangi-typography-selectWoo' ), MATANGI_VERSION, true );
			wp_enqueue_style( 'matangi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), MATANGI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'matangi_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'matangi' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'matangi' );
			$this->json[ 'google_fonts' ] = apply_filters( 'matangi_typography_customize_list', matangi_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = matangi_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'matangi' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'matangi' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'matangi' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'matangi' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="matangi-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="matangi-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="matangi-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="matangi-font-transform">
					<label>
