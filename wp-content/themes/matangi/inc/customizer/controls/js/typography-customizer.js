( function( api ) {
	...
	api.controlConstructor['matangi-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.matangi-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the matangi_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'matangi_get_all_google_fonts_ajax',
								matangi_customize_nonce: matangi_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.matangi-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.matangi-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.matangi-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.matangi-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.matangi-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.matangi-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.matangi-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.matangi-font-variant' );
			...
			control.container.on( 'change', '.matangi-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.matangi-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.matangi-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.matangi-font-family select' ).selectWoo();
	jQuery( '.matangi-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".matangi-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.matangi-font-variant' ).hide();
		}
	});
